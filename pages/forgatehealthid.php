...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($count == 1) {
        ...
        $sql="SELECT healthid FROM user where email= '$email' ";
        $result=$conn->query($sql);
        if($result->num_rows>0){
            while($row=$result->fetch_assoc()){
                   $healthid=$row["healthid"];
                   $smg="Your Healthid is :- ".$healthid;
                   ...
                   $mail->Subject = 'Health Id Recovery';
                   $mail->Body    = 'This is your <b style="color:red;">Health id</b> associated with email id '.$email.' is '.'<b>'. $healthid.'</b>' ;
                   ...
                   if (!$mail->send()) {
                       ...
                       $email_send="Your Health Id is sent to resistered email id";
                   }
         ...
         $err = "Invalid Name or email or dob.";
    }
}
        ...
        <div class="row d-lg-flex justify-content-lg-center" style="border-color: var(--bs-purple);border-left-color: var(--bs-pink);">
            <div class="col-4" style="height: 300px;">
                <div class="card" style="margin-top: 30px;background: var(--bs-white);box-shadow: 0px 0px 9px var(--bs-indigo);border-radius: 10px;">
                    <div class="card-body">
                        <h4 class="text-center card-title" style="font-family: Aclonica, sans-serif;color: var(--bs-pink);font-weight: bold;font-style: italic;">Forgate Healthid?</h4>
